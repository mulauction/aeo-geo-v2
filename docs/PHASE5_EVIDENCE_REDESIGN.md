...
**핵심 원칙: 기존 분석/저장/복원/렌더링 경로를 절대 건드리지 않는다.**
...
Phase5 Evidence의 목적:
- 분석 결과에 대한 상세한 근거(Evidence) 정보 제공
- 사용자가 점수 산정 근거를 명확히 이해할 수 있도록 지원
- 기존 점수 시스템과 독립적으로 Evidence 데이터 관리
...
**Do:**
- 기존 함수를 호출하여 데이터를 읽는 것
- 기존 함수의 반환값을 활용하는 것
- 새로운 함수 추가 (기존 함수 수정 없이)

**Don't:**
- 기존 함수의 내부 로직을 수정하는 것
- 기존 함수의 시그니처를 변경하는 것
- 기존 함수를 삭제하거나 대체하는 것

**예시:**
- Do: `getState()`를 호출하여 기존 상태를 읽기
- Do: `buildReportPayload()`를 호출하여 리포트 데이터 가져오기
- Don't: `setState()` 내부 로직을 수정하여 Evidence 저장 로직 추가
- Don't: `render()` 함수를 수정하여 기존 섹션 구조 변경

### 3.2 별도 네임스페이스 저장 원칙

**Evidence 데이터는 기존 데이터와 완전히 분리된 네임스페이스에 저장한다.**

Evidence 데이터는 `analysis.evidence` 네임스페이스에 저장하며, 기존 `analysis.scores`와는 독립적으로 관리한다.
...
**독립성 보장:**
- Evidence가 없어도 기존 기능은 정상 동작해야 함
- Evidence 계산 실패 시 기존 분석 결과에는 영향 없음
