    ...
    if (!isAuthed) {
      ...
      const shareButtonHtml = historyLength >= 2 ? `
        <div style="margin-top: 6px; margin-bottom: 8px; display: flex; justify-content: flex-end;">
          <button onclick="(() => { try { const urlParams = new URLSearchParams(location.search); const isDebug = urlParams.get('debug') === '1'; const debugParam = isDebug ? '&debug=1' : ''; let reportId = localStorage.getItem('__currentReportId'); if (!reportId) { const lastV2Str = localStorage.getItem('__lastV2'); if (lastV2Str) { const lastV2 = JSON.parse(lastV2Str); reportId = String(lastV2.generatedAt || Date.now()); localStorage.setItem('__currentReportId', reportId); } } const targetUrl = './share.html?restore=1' + debugParam; if (isDebug) console.info('[analyze] share nav from evidence', { target: targetUrl, reportId: reportId }); location.href = targetUrl; } catch (e) { console.warn('[analyze] share nav error:', e); location.href = './share.html?restore=1'; } })()" class="btn btn-primary" style="font-size: 12px; padding: 6px 12px;">최종 리포트(Share) 열기</button>
        </div>
      ` : '';
